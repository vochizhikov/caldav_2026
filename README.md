...
- Выбор календарей, просмотр встреч за ближайшие 1–7 дней со встречами, ручная синхронизация.
...
3. Отправьте боту логин Яндекса либо полный адрес почты и затем пароль приложения.
   Формат почты проверяется через Pydantic; адреса `@gmail.com` запрещены независимо от регистра.
   При ошибке бот попросит исправить логин, прежде чем запрашивать пароль.
   Основной пароль аккаунта не используйте.
4. Первый календарь в списке включается автоматически при подключении. В `/calendars`
   проверьте выбор и при необходимости включите остальные или выключите первый.
...
не помещает открытый пароль в FSM или логи и сохраняет только шифротекст Fernet. Если удалить сообщение
...
| `/settings` | Типы уведомлений, интервалы, часовой пояс и число дней ближайших событий |
...
В главном меню доступны «Мои календари» и «Ближайшие события». «Настройки уведомлений»
находятся в разделе «Мои календари», перед кнопкой отключения аккаунта. Кнопка
«← Мои календари» в настройках возвращает к списку календарей.

«Ближайшие события» и `/events` показывают будущие встречи за первый день со встречами
по умолчанию. В «Настройки уведомлений» → «🗓 Настройка ближайших событий» можно выбрать
от 1 до 7 дней для режима «Список»: выбранный вариант отмечен ✅ и сохраняется
для пользователя. Считаются даты в его часовом поясе, дни без встреч пропускаются.
Например, при выборе двух дней
встречи в понедельник и четверг попадут в список, даже если вторник и среда пусты.
Показываются все встречи за выбранные даты из кеша включённых календарей; горизонт
кеша по умолчанию — 45 дней. После списка сразу отправляется главное меню с кнопками,
отдельной кнопки «← Меню» у списка нет. Из настройки числа дней кнопка
«← Настройки уведомлений» возвращает к настройкам уведомлений.

Первой строкой этой клавиатуры всегда идёт кнопка «⬜ Режим каталога»; включённый режим
отмечен ✅. Каталог показывает первые 7 дней со встречами или меньше, если в кеше
недостаточно непустых дат, по одному дню в одном сообщении. Число дней обычного списка
на каталог не влияет. При включении каталога кнопки 1–7 скрываются; после выключения
возвращаются с сохранённым выбором. По умолчанию используется обычный список. В каталоге
стрелки ←/→ переключают дни, редактируя сообщение со встречами; главное меню
отправляется отдельно при открытии. У первого дня доступна только →, у последнего
только ←, а при одном дне стрелок нет. Даты фиксируются при открытии каталога:
синхронизация обновляет встречи, сохраняя выбранную дату страницы. Если день опустел,
каталог сообщает об этом. После смены часового пояса откройте ближайшие события заново.
Большой день отображается кратко; если все события всё равно не помещаются в сообщение,
бот указывает число неотображённых событий. Для полного списка выключите каталог,
выберите 7 дней в настройках и откройте ближайшие события заново.

Кнопка «Подключить Яндекс Аккаунт» отображается в главном меню у новых пользователей
и после отключения аккаунта. У подключённых пользователей она скрыта. Для повторного
подключения сначала отключите текущий аккаунт через `/disconnect` или кнопку
«🔌 Отключить Яндекс аккаунт» в списке календарей и подтвердите отключение.
...
При подключении аккаунта первый календарь в списке включается автоматически, остальные
остаются выключенными. После отключения и нового подключения это правило применяется снова.
Календари, найденные при последующих синхронизациях, добавляются выключенными;
сохранённый выбор пользователя не меняется. Если при подключении календарей ещё нет,
включите нужный в `/calendars`, когда он появится.
