...
`menu_keyboard(connected=False)` создаёт три постоянные строки кнопок: календари, ближайшие встречи, настройки. Их `callback_data`: `menu:calendars`, `menu:events`, `menu:settings`. Если аккаунт не подключён, добавляет кнопку «🔗 Подключить Яндекс аккаунт» с `menu:connect`. Обработчики передают `connected=bool(user.encrypted_password)`, поэтому у новых пользователей и после отключения кнопка видна, а у подключённых — скрыта.
...
3. Кнопку «Отключить Яндекс аккаунт» с `account:disconnect`.
4. Возврат в меню.
