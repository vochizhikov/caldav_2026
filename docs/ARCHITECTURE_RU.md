...
`show_events(message, session, user)`:

1. Проверяет наличие сохранённых учётных данных.
2. Вызывает `repositories.events.upcoming(...)` с текущим временем.
3. Получает до 20 отдельных встреч за следующие семь дней из включённых календарей.
4. Форматирует название, локальное время и название календаря.
...
**`connect_command()` и `connect_callback()`.** Вход через `/connect` или `menu:connect`. Очищают прежнее состояние, устанавливают `ConnectAccount.login` и просят логин. Существующее подключение на этом шаге ещё не удаляется.
...
Пароль не помещается в FSM. Он существует в локальной переменной во время проверки и шифруется перед сохранением. Неуспешная проверка новых данных не уничтожает прежнее рабочее подключение. Успешное переподключение создаёт календари заново, поэтому их нужно снова выбрать; пользовательские переключатели уведомлений сохраняются.
...
`menu_keyboard()` создаёт четыре строки кнопок: календари, ближайшие встречи, настройки, подключение Яндекса. Их `callback_data`: `menu:calendars`, `menu:events`, `menu:settings`, `menu:connect`.

`back_button()` возвращает кнопку главного меню с `menu:home`. Это общий элемент для других клавиатур.

### `bot/keyboards/events.py`

`events_keyboard()` возвращает клавиатуру с кнопкой назад. Логики выбора или загрузки событий здесь нет.
...
`upcoming(session, user_id, now)` соединяет `Occurrence` с `MyCalendar`, отбирает принадлежащие пользователю включённые календари и повторения в диапазоне:

```text
now <= starts_at < now + 7 дней
```

Сортирует по `starts_at`, ограничивает результат 20 строками и возвращает пары «повторение + календарь». Благодаря соединению обработчик сразу может показать название календаря рядом со встречей.
...
`/events` проходит через middleware, читает `Occurrence` через репозиторий и отправляет готовый список. `/calendars` читает состояния календарей из БД. `/settings` показывает текущие поля пользователя. Эти просмотры не являются проверкой доступности Яндекса в реальном времени.
...
При переподключении уже работающего аккаунта удаление старой копии выполняется только после успешной проверки новых учётных данных. Ошибка нового пароля не отключает старый аккаунт.
...
| Поменять список ближайших встреч | `bot/repositories/events.py` и текст `show_events()` |
