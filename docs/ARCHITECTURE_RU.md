...
13. Получает `user_calendars()` в порядке `id` и, если список непустой, включает первый календарь. Остальные остаются выключенными.
14. Делает `commit()`, очищает FSM, сообщает о подключении и показывает список календарей. При непустом списке дополнительно сообщает, что первый календарь включён по умолчанию.

Пароль не помещается в FSM. Он существует в локальной переменной во время проверки и шифруется перед сохранением. Для повторного подключения пользователь сначала подтверждает отключение текущего аккаунта. Новое подключение создаёт календари заново и снова включает первый; пользовательские переключатели уведомлений сохраняются. Если Яндекс вернул пустой список, подключение сохраняется без выбранных календарей: найденные позже календари нужно включить вручную.

Первый календарь включается только в обработчике подключения. Ручное выключение сохраняется при последующих синхронизациях. Немедленной загрузки событий здесь нет: календарь остаётся с `initialized=False` до следующего фонового прохода или `/sync`, который создаст тихий начальный снимок.
...
- `validate_login(value)` проверяет ввод через Pydantic-модель `LoginInput`: обрезает пробелы по краям, требует непустой текст длиной до 320 символов, запрещает пробельные символы внутри, `:` и угловые скобки. Короткий логин Яндекса также разрешён. Если есть `@`, формат адреса проверяется через `EmailStr`, а домен нормализуется. Домен `gmail.com` запрещён независимо от регистра; список ограничений хранится в `_BLOCKED_EMAIL_DOMAINS`. Проверка не обращается к DNS и не доказывает существование аккаунта. Ошибки Pydantic преобразуются в короткий `ValueError` без пользовательского ввода, и бот остаётся ждать исправленный логин.
...
2. Для нового удалённого календаря добавляет `MyCalendar`. ORM по умолчанию выставляет `enabled=False` и `initialized=False`. Только обработчик подключения после `reconcile()` отдельно включает первый календарь.
...
| `enabled` | Выбор пользователя, ORM по умолчанию задаёт **`False`**; при подключении обработчик включает первый календарь |
...
- `test_onboarding_settings_and_account_isolation`: `/start`, подключение с одним или тремя календарями, удаление сообщения с паролем, шифрование, автоматическое включение первого календаря при выключенных остальных, отсутствие немедленной загрузки событий, настройки интервала, запрет чужого календаря, сохранение ручного выключения после `/sync`, повторное включение, отключение аккаунта и выбор первого календаря при новом подключении.
...
sequenceDiagram
    ...
    V-->>H: Шифротекст
    H->>DB: Аккаунт + календари, первый включён при наличии
    H->>H: Очистить FSM
    H->>U: Подключено + список календарей
```
...
При подключении аккаунта первый календарь включается автоматически и остаётся с `initialized=False`. Остальные можно включить кнопкой в списке:
...
После этого `connected_ids()` больше не выбирает пользователя. Новый `/connect` выполняет обычное подключение: первый найденный календарь включается автоматически, остальные остаются выключенными.
